...
export default function ServicesPage() {
  return <main className="services-index"><header className="site-header"><Link className="brand" href="/"><span className="brand-mark"><Home size={18} /></span><span>False Ceiling<br /><strong>Service</strong></span></Link><nav aria-label="Main navigation"><Link href="/">Home</Link><Link href="/services" aria-current="page">Services</Link><Link href="/#projects">Projects</Link><Link href="/#contact">Contact</Link></nav><a className="header-cta" href="tel:+917258951213">Call Now / +91 7258951213</a></header><section className="services-index-hero section-shell"><Link className="back-link" href="/">← Back to home</Link><div className="section-kicker">Our services</div><h1>One place for every<br /><em>interior finish.</em></h1><p>Explore our complete range of ceiling, wall and interior services for homes, offices and shops across Chandigarh. Every recommendation starts with your room, usage, finish preference and budget.</p><div className="services-index-note"><Check size={19} /> Choose a service to compare its use, pricing guidance, timeline and finish benefits.</div></section><section className="services-index-grid section-shell">{services.map(([title, hindi, slug, price, text, Icon]) => <Link href={`/services/${slug}`} className="service-card" key={slug}><div className="service-icon"><Icon size={21} /></div><div className="service-card-top"><h2>{title}</h2><span>{price}</span></div><p>{text}</p><span className="service-card-link">View service details <ChevronRight size={15} /></span></Link>)}</section><section className="services-index-cta section-shell"><div><div className="section-kicker">Need help choosing?</div><h2>Tell us about<br /><em>your space.</em></h2></div><Link className="button button-primary" href="/#contact">Get a quote <ArrowRight size={17} /></Link></section></main>
}
